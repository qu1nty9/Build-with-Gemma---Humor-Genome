...
После локального latency spike выбран следующий model split: **Gemma 4 E2B** — основной интерактивный runtime, **Gemma 4 12B instruction-tuned** — offline quality benchmark. 12B дала качественный структурный анализ, но на 16-ГБ M1 Pro потребовала 51,6 секунды только для стадии `analyze`, поэтому не подходит для live hero-flow. Если E2B не даст приемлемого controlled output, fallback — Gemma 3 4B для text-first MVP; 12B остаётся эталонным «teacher/judge» вариантом.
